...
# Keep identifiers separate: Ticker is trading symbol; ISIN is product identifier; Name is display name.
TICKER_COLUMNS = ["ticker", "symbol", "kode", "handelskode", "trading symbol", "yahoo ticker"]
ISIN_COLUMNS = ["isin", "isin kode", "isin-kode", "isin code"]
NAME_COLUMNS = ["instrument", "navn", "name", "etf navn", "etf name", "produkt", "security", "description"]

# Exposure should be current market value, not quantity * price unless market value is missing.
EXPOSURE_COLUMNS = [
    "aktuel beholdning",
...
QUANTITY_COLUMNS = ["antal", "quantity", "shares", "stk", "units"]
PRICE_COLUMNS = ["dags kurs", "kurs", "price", "last", "close", "seneste kurs"]
...
def clean_number(value):
    ...
    if isinstance(value, (int, float)):
        return float(value)
    text = str(value).replace("\xa0", "").replace(" ", "").replace(".", "").replace(",", ".").replace("%", "").strip()
    try:
        return float(text)
    except ValueError:
        return None
...
def standardize_portfolio(df: pd.DataFrame, default_source: str = "Uploaded") -> pd.DataFrame:
    ...
    if ticker_col is None and isin_col is None and name_col is None:
        raise ValueError("Jeg kan ikke finde ticker, ISIN eller instrument/navn i filen.")

    out = pd.DataFrame(index=df.index)
    out["ETFName"] = df[name_col].map(clean_text) if name_col else ""
    out["ISIN"] = df[isin_col].map(clean_text) if isin_col else ""

    if ticker_col:
        out["Ticker"] = df[ticker_col].map(clean_text)
    elif isin_col:
        out["Ticker"] = out["ISIN"]
    else:
        out["Ticker"] = out["ETFName"]
    ...
    out["Sector"] = df[sector_col].map(clean_text) if sector_col else "Ukendt"
    ...
    # Fallback only if true market value/current holding value is missing.
...
def load_default_files(data_dir: str | Path = "data") -> pd.DataFrame:
    ...
    if not frames:
        return pd.DataFrame(columns=["Ticker", "ISIN", "ETFName", "Exposure", "Sector", "Quantity", "InputPrice", "Source", "Weight"])
